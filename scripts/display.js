...
export default class AttackDisplay {
    ...
    update(tokenId, roll) {
        ...
        if ((game.settings.get(scsApp.ID, "hudDisplay") || game.settings.get(scsApp.ID, "trackerDisplay")) && scsApp.phases.names[scsApp.currentPhase - 1] === "Attacks") {
            ...
            canvas.tokens.placeables.filter(token => token.id === tokenId).forEach(token => {
                ...
                if (this.DISPLAY[tokenId].tracker) this.DISPLAY[tokenId].tracker.forEach(d => d.text.innerText = roll);
    ...
    remove(tokenId) {
        ...
        if (this.DISPLAY[tokenId]) {
            ...
            this.DISPLAY[tokenId].tracker?.forEach(d => { d.icon.remove(); d.text.remove(); });
